// Filename: ELEC3004_Lab1_Ex04_EchoFeedback.ino
...
uint16_t y_arr[N];            // Array storing past samples of output signal
...
void loop() {
  ...
  uint16_t n_delay = (n + N - D) % N;     // Index corresponding to y[n - D]
  uint16_t y_delay = y_arr[n_delay];      // Delayed sample y[n - D]
  
  uint16_t y = (x + y_delay) / 2;         // y[n] = 0.5*x[n] + 0.5*y[n - D]
  y_arr[n] = y;                           // Store current output sample in array
  analogWrite(DAC1, y);
